...
describe('TabsPage', () => {
  ...
  it('should have the ion-tab-bar at the bottom', () => {
    const tabBar = fixture.nativeElement.querySelector(
      'ion-tab-bar'
    ) as HTMLIonTabBarElement;
  ...
  it('should have all ion-tab-button with icons and labels only', () => {
    const tabButtons = fixture.nativeElement.querySelectorAll(
      'ion-tab-button'
    ) as Array<HTMLIonTabButtonElement>;
  ...
  it('should set active icon only for active tab', () => {
    ...
    component.tabs.forEach((tab, index) => {
      ...
      const icons = Array.from(
        fixture.nativeElement.querySelectorAll('ion-icon') as NodeList
      ) as Array<HTMLIonIconElement>;
      icons.forEach((icon) => {
        const isInactive =
          inactiveIconNames.indexOf(icon.name?.toString() as string) !== -1;
        const isActive =
          activeIconNames.indexOf(icon.name?.toString() as string) !== -1;
  ...
  it('should call tab page method on load', () => {
    component.tabs.forEach((tab: Tab, index) => {
      if (!tab.onLoad) return;
      const navigationEvent = new NavigationEnd(index, tab.tabName, '');
      tab.onLoad = jasmine.createSpy();
      component.executeMethodOnPageChange(navigationEvent);
      fixture.detectChanges();
      expect(tab.onLoad).toHaveBeenCalled();
    });
  });
});
